...
class BaseStation(Node):
    def __init__(self):
        ...
        super().__init__("testbed_bs")

        self.create_timer(0.25, self.send_controls)


        # Create a publisher to publish any output the pico sends
        self.publisher = self.create_publisher(String, '/testbed/controls', 10) 

        # Create a subscriber to listen to any commands sent for the pico
        self.subscriber = self.create_subscription(String, '/testbed/feedback', self.read_feedback, 10)
        self.subscriber
